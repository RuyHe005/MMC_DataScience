...
-- cau 3: 
SELECT DepartmentID FROM Department WHERE DepartmentName = 'PhongSale';

-- cau 4: 
SELECT * FROM ACCOUNTS ORDER BY LENGTH(FULLNAME) DESC LIMIT 1;

-- cau 5: 
SELECT * FROM ACCOUNTS WHERE DepartmentID = 03 ORDER BY LENGTH(FULLNAME) DESC LIMIT 1;

-- cau 6: 
SELECT GROUP_NAME FROM GROUP_NHOM WHERE CREATE_DATE < '2019-12-20';

-- cau 7: 
SELECT QUESTIONID FROM ANSWER GROUP BY QUESTIONID HAVING COUNT(*) >= 4;

-- cau 8: 
SELECT CODE FROM EXAM WHERE TIME_TO_SEC(DURATION) >= 3600 AND CREATEDATE < '2019-12-20';

-- cau 9: 
SELECT * FROM GROUP_NHOM ORDER BY CREATE_DATE DESC LIMIT 5;

--  cau 10: 
SELECT COUNT(*) FROM ACCOUNTS WHERE DepartmentID = 02;
...
--  cau 14:
UPDATE ACCOUNTS SET FULLNAME = 'Lô Văn Đề', Email = 'lo.vande@mmc.edu.vn' WHERE AccountID = 5;

--  cau 15: 
UPDATE GROUP_ACCOUNT SET GROUPID = 224 WHERE ACCOUNTID = 115;
```
